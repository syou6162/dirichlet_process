...
public:
  ...
  double make_cumsum_vector(std::vector<double>& posts, int index) {
	...
	return logsumexp(psum, posts[new_cluster_id], new_cluster_id == 0);
  };
  ...
  void initialize() {
	...
	std::cout << "Number of clusters is " << get_num_of_clusters() << std::endl;
  }
...
protected:
  ...
  std::map<int, int> Nk_; // Number of customers in table k
  std::map<int, std::map<int, int> > Nkw_; // Number of customers in table k
